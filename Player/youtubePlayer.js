...
// Production-ready, minimal, async/await, GitHub Pages compatible
...
(() => {
  ...
  async function loadCore(CFG) {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.onload = () => window.LibreUltra ? resolve() : reject('LibreUltra failed');
  ...
  function startSponsorWatcher(iframe) {
    const yt = iframe.contentWindow;
    if (!yt || !yt.YT) return;
    sponsorInterval = setInterval(() => {
      const t = yt.YT?.getPlayers?.()?.[0]?.getCurrentTime?.();
      if (!t) return;
      for (const seg of sponsorSegments) {
        const [start, end] = seg.segment;
        if (t >= start && t < end) { yt.YT.getPlayers()[0].seekTo(end, true); break; }
      }
    }, 300);
  }
  ...
  function destroyPlayer() { clearPlayer(); }

  // Expose to global for GitHub Pages usage
  window.LibrewatchPlayer = { createYouTubePlayer, destroyPlayer };
})();
